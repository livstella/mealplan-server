...
const categoryRecipe = (req, res, next) => {
  pool
    .query(
      "SELECT category.name, recipe_id FROM category_recipe FULL JOIN category ON category_id=category.id INNER JOIN recipe ON recipe.id=recipe_id;"
    )
    .then((data) => res.json(data.rows))
    .catch((e) => console.log(e));
};
...
const ingredientUnit = (req, res, next) => {
  pool
    .query(
      "SELECT ingredient.name, unit.name, amount, recipe_id FROM ingredient_unit FULL JOIN unit ON unit_id=unit.id FULL JOIN ingredient ON ingredient_id=ingredient.id"
    )
    .then((data) => res.json(data.rows))
    .catch((e) => console.log(e));
};
...
const getRecipes = async (req, res, next) => {
  try {
    const result = await pool.query(
      "SELECT recipe.id, recipe.name, recipe.img_url, recipe.description, author_site.name AS author FROM recipe FULL JOIN author_site on author_id=author_site.id;"
    );
...
const getIngredients = async (req, res, next) => {
  try {
    const result = await pool.query(
      "SELECT  amount, unit.name AS unit,  ingredient.name AS ingredient, recipe_id FROM ingredient_unit FULL JOIN unit ON unit_id=unit.id FULL JOIN ingredient ON ingredient_id=ingredient.id"
    );
...
const getCategories = async (req, res, next) => {
  try {
    const result = await pool.query(
      "SELECT * FROM category_recipe FULL JOIN category ON category_id=category.id;"
    );
...
const savedRecipes = (req, res, next) => {
  pool
    .query(
      "SELECT user_id, recipe.id, recipe.name, recipe.img_url, recipe.description, author_site.name AS author FROM saved_recipes FULL JOIN recipe ON recipe.id=recipe_id FULL JOIN author_site on author_id=author_site.id;"
    )
    .then((data) => res.json(data.rows))
    .catch((e) => console.log(e));
};
